...
class SphereTest {
    ...
    void findIntersections() {
        ...
        var result = sphere.findGeoIntersectionsHelper(new Ray(new Point(3, 3, 0), new Vector(-1, -1, 0)));
        ...
        result = sphere.findGeoIntersectionsHelper(new Ray(new Point(1, 1, 0), new Vector(-1, -1, 0)));
        assertEquals(1, result.size(), "Error: there should be one intersection point");

        // The ray begins in Sphere and there is no harm
        assertNull(sphere.findGeoIntersectionsHelper(new Ray(new Point(3, 3, 0), new Vector(1, 1, 0))),
        ...
        assertEquals(null,
                sphere.findGeoIntersectionsHelper(new Ray(new Point(2, 0, 1), new Vector(1, 0, 0.5))),
                "Error: there are not intersection points");

        // Starts on the edge and goes inside there is one hit
        result = sphere.findGeoIntersectionsHelper(new Ray(new Point(2, 0, 1), new Vector(-1, -0.5, 0)));
        assertEquals(1,
                result.size(),
        ...
        assertEquals(null,
                sphere.findGeoIntersectionsHelper(new Ray(new Point(2, -1, 1), new Vector(0, 1, 0))),
                "Error: there are not intersection points");

        // The ray starts the sphere and does not damage it
        assertEquals(null,
                sphere.findGeoIntersectionsHelper(new Ray(new Point(2, 0, 1), new Vector(0, 1, 0))),
                "Error: there are not intersection points");
        ...
        result = sphere.findGeoIntersectionsHelper(new Ray(new Point(0, 0, 1), new Vector(1, 1, 0)));
        ...
        assertEquals(null,
                sphere.findGeoIntersectionsHelper(new Ray(new Point(0, -2, 1), new Vector(0, -1, 0))),
                "Error: there are not intersection points");

        //The ray starts on the count and goes through the center and there is one hit
        result = sphere.findGeoIntersectionsHelper(new Ray(new Point(0, 2, 1), new Vector(0, -1, 0)));
        assertEquals(1, result.size(), "Error: there should be one intersection points");

        //The ray starts after the sphere and if we reverse the direction it will go through the center
        assertEquals(null,
                sphere.findGeoIntersectionsHelper(new Ray(new Point(0, -3, 1), new Vector(0, -1, 0))),
                "Error: there are not intersection points");

        //The ray starts in front of the shpere and goes through the center

        result = sphere.findGeoIntersectionsHelper(new Ray(new Point(0, -1, 1), new Vector(0, -1, 0)));
        assertEquals(1, result.size(), "Error: there should be one intersection points");

        //The ray starts inside the sphere and if we reversed the direction it would pass through the center
        result = sphere.findGeoIntersectionsHelper(new Ray(new Point(0, 3, 1), new Vector(0, -1, 0)));
        assertEquals(2, result.size(), "Error: there should be two intersection points");

        //If you draw a line from the center point to the top of the horn then it is perpendicular to the horn and it is
        // outside the sphere
        assertEquals(null,
                sphere.findGeoIntersectionsHelper(new Ray(new Point(2.5, 0, 1), new Vector(0, 1, 0))),
                "Error: there are not intersection points");

        //If you draw a line from the center point to the top of the ray then it is perpendicular to the ray and it is
        // inside the sphere
        result = sphere.findGeoIntersectionsHelper(new Ray(new Point(1.5, 0, 1), new Vector(0, 1, 0)));
        assertEquals(1, result.size(), "Error: there should be one intersection points");
    }
}